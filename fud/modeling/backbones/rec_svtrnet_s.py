...
class SVTRNet(nn.Layer):
    def __init__(
            ...
            depth=[3, 6, 6],
            num_heads=[3, 6, 8],
            mixer=['Local'] * 8 + ['Global'] * 7,  # Local atten, Global atten, Conv
    ...
    def forward(self, x):
        ...
        if self.last_stage:
            ...
            x = self.avg_pool(
                x.transpose([0, 2, 1]).reshape(
                    [0, self.embed_dim[2], h, self.HW[1]])) # [128, 256, 1, 40]
            x = self.last_conv(x) #[128, 192, 1, 25]
            x = self.hardswish(x)
            x = self.dropout(x)
...
# x = paddle.Tensor(np.random.rand(128, 3, 32, 320).astype(np.float32))
# model = SVTRNet()
#
# import numpy as np
#
...
#
# calculate_params(model) #9358440 9358440 0
#
# import time
# # 记录开始时间
